...
classes_dog_and_cat = np.arange(0, 294, dtype=np.int32)
class_to_index = dict()
for i, c in enumerate(classes_dog_and_cat):
    class_to_index[c] = i


class StanfordDogDataset(chainer.dataset.DatasetMixin):
...
if __name__ == "__main__":
    ...
    dirname_to_label = {}
    with open('dogdirname_to_label.txt', 'r') as f:
        for line in f:
    ...
    np.savetxt('image_list_stanford_dog.txt', n_image_list, fmt="%s")
    ...
    if preprocess_val:
        ...
        print("Num of examples:{}".format(count))
        n_image_list = np.array(n_image_list, np.str)
        np.savetxt('image_list_stanford_dog_val.txt', n_image_list, fmt="%s")
